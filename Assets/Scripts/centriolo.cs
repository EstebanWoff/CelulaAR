...
public class centriolo : MonoBehaviour
{
    ...
    private float highlightTime = 28f; // tiempo en segundos que el objeto estará resaltado
    public Material highlightColor; // color de resaltado del objeto

    public Material centriolosM;
    ...
    private float displayTime = 28f; // tiempo que se muestra el panel en segundos
    ...
    void Start()
    {
        ...
        allChildren = new GameObject[objectToTouch.transform.childCount];
        for (int i = 0; i < allChildren.Length; i++)
        {
            allChildren[i] = objectToTouch.transform.GetChild(i).gameObject;
        ...
        if (Physics.Raycast(finger.GetRay(), out hit))
        {
...
)
            {
                ...
                if (!objectHighlighted)
                {
                    for (int i = 0; i < allChildren.Length; i++)
                    {
                        allChildren[i].GetComponent<Renderer>().material = highlightColor;
                    }
    ...
    void ResetObjectHighlight()
    {
        ...
        for (int i = 0; i < allChildren.Length; i++)
        {
            allChildren[i].GetComponent<Renderer>().material = centriolosM;
